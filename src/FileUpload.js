...
const FileUpload = React.createClass({
  ...
  _updateProps(props) {
    ...
    if (this.chooseAndUpload) {
      React.Children.forEach(props.children, (child) => {
        if (child && child.className.find('chooseAndUpload') !== -1) {
          chooseBtn = child
          flag++
        } else {
      ...
      React.Children.forEach(props.children, (child) => {
        ...
        if (child && child.className.find('chooseBtn') !== -1) {
          chooseBtn = child
          flag++
        } else if (child && child.className.find('uploadBtn') !== -1) {
          uploadBtn = child
          flag++
        } else {
  ...
  commonUpload() {
    ...
    xhr.onerror = () => {
      try {
        const resp = this.dataType == 'json' ? JSON.parse(xhr.responseText) : xhr.responseText
        this.uploadError({type: 'XHRERROR', message: resp, mill})
      } catch (e) {
        this.uploadError({type: 'XHRERROR', message: e.message, mill})
      }
    }
